...
export default function Cuestionario({ version, demo = false }) {
  ...
  const handleEnviar = async () => {
    ...
    // ── Deduplicidad por email (si se proporcionó)
    if (emailContacto && version === 'D') {
      const { data: emailExistente } = await supabase
        .from('respuestas_dir')
        .select('id')
        .eq('email_contacto', emailContacto.toLowerCase().trim())
        .limit(1)
      if (emailExistente && emailExistente.length > 0) {
        setErrorEnvio('Ya existe una respuesta registrada con este email. Si cree que es un error, contacte con el investigador.')
        setEnviando(false)
        return
      }
    }
    ...
    fila.email_contacto            = emailContacto?.toLowerCase().trim() || null
      ...
      <div className="tarjeta-cuerpo">
        ...
        {!dimensionCompleta(dim) && (
          <p style={{ fontSize: '0.78rem', color: '#555e7a', marginTop: 10, textAlign: 'center' }}>
            ...
            (puede usar NS/NC si no dispone de información suficiente).
          </p>
        )}
