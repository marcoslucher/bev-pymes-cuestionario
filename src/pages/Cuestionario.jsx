...
export default function Cuestionario({ version, demo = false }) {
  ...
  const [paso,       setPaso]       = useState(0)   // 0=clasificación, 1-10=dimensiones, 11=cierre
  ...
  useEffect(() => {
    if (demo) { setEmpresa({ codigo: 'DEMO', nombre: 'Empresa Demo', estrato: null, sector: null }); setCargando(false); return }
  ...
  const handleEnviar = async () => {
    ...
    // Validación: evitar respuesta duplicada (solo para versión D)
    if (version === 'D') {
    ...
    const fila = {
      empresa_codigo: codigo.toUpperCase(),
      ...clasificacion,
      disponibilidad_ampliacion: disponibilidad,
      email_contacto: emailContacto || null,
    }
    DIM_KEYS.forEach(dim => {
      const n = dim.toLowerCase()
      ;[1,2,3,4,5,6].forEach(i => { fila[`${n}_${i}`] = respuestas[`${dim}_${i}`] || null })
    })

    // Actualizar datos empresa si es directivo
      ...
      <div className="tarjeta-cuerpo">
        ...
        {paso === 0 && (
          <div className="escala-explicacion" style={{ marginTop: 16 }}>
            ...
            <p>Todas las secciones utilizan la misma escala. Le pedimos que valore la
              <strong> importancia estratégica</strong> de cada aspecto para su empresa u organización,
        ...
        <DatosClasificacion
          ...
          onComplete={(datos) => { setClasificacion(datos); setPaso(1) }}
        />
      </div>
    </div></div>
  )
      ...
      <div className="tarjeta-cuerpo">
        ...
        <div className="campo-grupo">
          <label className="campo-label">
            ¿Estaría disponible para participar en una posible ampliación del estudio? <span style={{ color: '#e53e3e' }}>*</span>
          </label>
          {['Sí, con mucho gusto', 'Posiblemente', 'No, prefiero no participar en más fases'].map(op => (
            <label key={op} style={{
              ...
              <input type="radio" name="disponibilidad" value={op}
                checked={disponibilidad === op} onChange={() => setDisponibilidad(op)} />
              {op}
            </label>
          ))}
        </div>
        ...
        <div className="campo-grupo" style={{ marginTop: 20 }}>
          <label className="campo-label">
            Email de contacto <span style={{ fontSize: '0.82rem', color: '#888', fontWeight: 400 }}>(opcional — solo para enviarle el informe de resultados)</span>
          </label>
          <input type="email" className="campo-input" placeholder="ejemplo@email.com"
            value={emailContacto} onChange={e => setEmailContacto(e.target.value)} />
        </div>

        {errorEnvio && (
          <div style={{ color: '#e53e3e', fontSize: '0.88rem', marginTop: 12 }}>⚠ {errorEnvio}</div>
        )}
  ...
  return (
    <div className="contenedor"><div className="tarjeta">
      ...
      <div className="tarjeta-cuerpo">
        ...
        {/* Nota D9 — instrucción contextual, sin filtro */}
        {dim === 'D9' && dimData.nota && (
        ...
        {/* Explicación de escala solo en D1 */}
        {paso === 1 && (
        ...
        <div className="botones-nav">
          <button className="btn btn-secundario" onClick={() => setPaso(p => p - 1)}>← Anterior</button>
          <button
            className="btn btn-primario"
